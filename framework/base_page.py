...
logger = LoggerRecord("BasePage").getlog()
print("BasePage logger为%s" % logger)
...
class BasePage(object):
    ...
    def get_windows_img(self):
        file_path = dirname(abspath(".")) + '\screenshots\\'
        rg = time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))
        screen_name = file_path + rg + '.png'

        try:
            self.driver.get_screenshot_as_file(screen_name)
            logger.info("Had take screenshot and save to foder : \screenshots")
        except NameError as e:
    ...
    def find_element(self, selector):
        ...
        elif selector_by == 'l' or selector_by == 'link_test':
    ...
    def click_selector(self, selector):
        ...
        try:
            el.click()
            logger.info(r"The element '%s' was clicked." % el.text)
        except NameError as e:
    ...
    def sleep(seconds):
        time.sleep(seconds)
        logger.info("Sleep fpr %d seconds" % seconds)
